...
## Core Components
- `cmd/main.go`
  Starts the controller manager, health/ready probes, metrics endpoint, and registers all reconcilers. Determines the operator namespace from the in-cluster service-account namespace file (`/var/run/secrets/kubernetes.io/serviceaccount/namespace`) and restricts most watches/writes to that namespace. Registers the reconcilers of every CRD, plus the cluster-wide `NodeDrainCoordinator`.
- `cmd/simplyblock-rebalancer/`
  ...
  - `--mode=probe`: long-running Prometheus exporter that watches a ConfigMap-backed node config and exposes `simplyblock_node_fio_write_latency_p50_ns` / `_p99_ns` gauges (labeled by `cluster`/`node`) on `:9199` (default).
...
- `api/v1alpha1/*`
  ...
  - `VolumeMigration`
- `internal/controller/*`
  Reconciliation logic for each CRD plus the cluster-wide `NodeDrainCoordinator`.
- `internal/controllers/backup/*`
  The data-protection band: the `StorageBackup` mirror that turns what a cluster's store holds into one object per backup, the `StorageBackupPolicy` reconciler that puts a schedule into the control plane and keeps its membership equal to what its selector matches, and the `StorageBackupOps` reconciler that restores one backup into a new claim.
- `internal/webapi/*`
...
- `internal/utils/*`
  ...
  - status/action constants and formatting helpers (for example, `CSIProvisioner = "csi.simplyblock.io"`)
- `internal/rebalancer/*`
...
- `StorageBackup`
  One point-in-time copy of one volume in the cluster's store, discovered rather than declared: the operator creates one object per backup the control plane reports, and a validating webhook refuses a create and a delete from every identity but the operator's. Deleting the object never deletes the copy.
- `StorageBackupPolicy`
  Schedules and retains the backups of the claims its `spec.claimSelector` matches, by reconciling the policy into the control plane and attaching each matching claim's volume to it. An absent selector selects nothing, which is deliberate: the cost of backing up too much is silent and recurring. The control plane runs the schedule and the pruning.
- `StorageBackupOps`
  One operation against one `StorageBackup`, which today is `Restore`. It is driven by a declared `atlas-lib/statemachine` graph through `Validating → Restoring → AwaitingVolume → Binding`, takes `StorageBackup.status.activeOpsRef` as its lock so that two restores of one backup queue rather than run together, and refuses a claim name that already exists. The claim it produces carries no owner reference back to it, so deleting the audit record leaves the recovered volume alone.
- `BackupRestore`
...
- Controller-layer tests were expanded from scaffold-level checks to branch-driven reconcile and state-transition tests across StorageCluster, StorageNodeSet, Pool, Task, the data-protection band, SnapshotReplication, VolumeMigration, and NodeDrain paths.
