...
The **simplyblock operator** turns the simplyblock control plane into a set of native Kubernetes
Custom Resources, so you can provision and operate **high-performance NVMe/TCP block storage** the
...
Where the [CSI driver](../csi-driver/README.md) provisions and attaches volumes to workloads, the
...
```sh
kubectl apply -f https://raw.githubusercontent.com/simplyblock/simplyblock-operator/main/operator/dist/install.yaml
...
**Build and deploy from source** (run from the `operator/` directory):
