...
type StorageBackupSpec struct {
	...
	// SnapshotName optionally overrides the snapshot name the operator would otherwise choose.
	// +optional
...
type StorageBackupStatus struct {
	...
	// FSType is the filesystem type of the source PersistentVolume ("ext4," "XFS"),
	// captured at backup time so a restore can preserve it regardless of
	// which StorageClass the restored PVC ends up using.
	FSType string `json:"fsType,omitempty"`

	// SnapshotID is the UUID of the snapshot the operator created used for the backup request.
