...
Every key this feature reads or writes uses the `storage.simplyblock.io/` prefix: the `storage.simplyblock.io/consistency-group` membership label and the node and logical volume store placement-pin annotations alike, so all of them sit in one family. Re-seating the existing `simplyblock.io/`-prefixed placement pins onto it is a deliberate one-time break of any manifest that sets the old keys.
...
| A labeled volume would exceed the group's 20-member cap                                                               | Backend rejects the volume create         | The `CreateVolume` fails before the volume is created, the PVC stays Pending with the cap error; the group stays at its cap (§4.1).                                                                                                            |
