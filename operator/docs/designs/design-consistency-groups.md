...
### 9.5 Admission webhook: a consistency-group member cannot be migrated

Migration excludes group members (§8.4), and the operator enforces that at admission rather than leaving it to the backend alone. A validating webhook on `VolumeMigration` create resolves the object's `spec.pvName` to its backing lvol through the volume handle on the `PersistentVolume`, asks the backend whether that lvol is a consistency-group member (`GET .../volumes/{id}` returns its `group_id`), and rejects the create when it is, naming the volume and its group. The operator therefore never starts a migration that would move a member off its group's pinned store, so a doomed `VolumeMigration` is refused at `kubectl apply` instead of running until the backend aborts it partway.

Because a migration moves the whole NVMe subsystem, every sibling volume sharing it moves too (§10 status `memberCount`), so the webhook rejects the create when the named volume or any sibling in its subsystem is a group member. It fails open the same way §9.4 does: when it cannot determine membership because the backend is unreachable, it admits the object, and the backend refusal is the backstop.

The webhook needs `get` on `persistentvolumes`, a `ValidatingWebhookConfiguration` targeting `volumemigrations`, and the operator's existing serving certificate. Migrating a whole group as a unit, moving the shared pin together so every member stays colocated, is a later option and is Open Question 2, at which point this webhook admits a group migration that keeps the members colocated rather than refusing it outright.
...
| `GET`    | `/api/v2/clusters/{id}/storage-pools/{pid}/volumes/{vid}`        | Existing volume read, extended to surface `group_id` (and `group_seq`) on the volume record. The migration webhook (§9.5) uses it to decide whether a volume is a consistency-group member.                                         |
...
An operator applies a `VolumeMigration` to move member V to another node. The operator's admission webhook (§9.5) resolves V to its backing lvol, finds it is a consistency-group member, and rejects the create at `kubectl apply`, naming V and its group, so no `VolumeMigration` object is created and the operator never starts the move. Were the request to reach the backend by some path the webhook does not cover, the backend refusing to migrate a group member is the last line of defense behind it (§8.4). Either way the group's placement stays fixed, so every generation can always be frozen on one store. Migrating a whole group as a unit is a later option (Open Question 2).
...
| 3   | ~~**Headless group clone.**~~ **Resolved:** `sbctl` offers a one-call `sbctl consistency-group clone <gid> <seq>` that clones every member of a generation into a new set in one command (§7.3), rather than leaving group clone to N per-member clones. The Kubernetes path stays per-member, because the CSI specification has no group-clone verb.                                                                                                                                                                  | Resolved                      |
| 4   | ~~**Label key prefix.**~~ **Resolved:** every annotation and label this feature reads or writes uses the `storage.simplyblock.io/` prefix, and the placement-pin annotations re-seat onto it too, so all keys sit in one family. This re-seats the existing `simplyblock.io/`-prefixed placement pins, a deliberate one-time break of manifests that set the old keys.                                                                                                                                                 | Resolved                      |
