...
// node. Applies to every storage node in the cluster, and no node carries a copy
// of it. Required: it sizes huge pages, and a node that receives no value fails
...
**The two are read differently, and which of them a node copies follows from what
each describes.** `vcpuCount` describes the host a node runs on, so each
`StorageNode` carries its own in `spec.config.sizing`, stamped from here when the
node is created ([`design-storagenode.md`](design-storagenode.md) §3.1).
`maxSubsystemCount` describes the cluster: it bounds how many volumes any node can
serve, which is an input to where volumes can be placed, so no node carries a copy
and every node's configuration is generated from this field
([`design-storagenode.md`](design-storagenode.md) §5.3). A copy of it could only
repeat the cluster's value, and the one thing it could do beyond that is disagree.

**Both are stated here and nowhere else.** A deployment config carries all three
sizing values in its cluster block and gives a node set none of its own
([`design-clusterdeploymentconfig.md`](design-clusterdeploymentconfig.md) §3.1),
so a fleet whose nodes disagree is a fleet the operator is part-way through
re-sizing rather than one a document described.

**What is stamped is equal across the fleet in steady state, and a rolling
hardware upgrade is what makes two nodes differ.** Replacing a fleet with larger
machines is done one node at a time, and a node moved to a host with more cores is
re-sized when it moves, so for the duration of the roll the cluster's nodes
genuinely differ. A model that could not express that would force the whole fleet
to be re-sized at once or not at all. `maxSubsystemCount` is outside that, because
a larger host does not change how many subsystems the cluster gives a node.
...
Everything outside that table is mutable, including the sizing group, and the two
sizing fields behave differently when they are edited. Changing `vcpuCount` on a
live cluster is accepted by the API server and changes what the next node created
is stamped with, rather than re-sizing the nodes that exist (§3.1). Re-sizing
those is an operation, because a node's layout is generated from its own copy and
changing it under a running node is restart-shaped rather than declarative.
Changing `maxSubsystemCount` reaches every node instead, since no node holds a
copy: the next pass rewrites each node's configuration entry and each node picks
the new cap up when it next restarts
([`design-storagenode.md`](design-storagenode.md) §5.3). The edit is declarative
and its effect is not immediate, which is the shape of every value a node reads at
start.
...
| Sizing read live by every node                                                      | `vcpuCount` and `minHugePagesSize` stamped onto a node at creation (§3.1) | Behavioral. An edit to either stops reaching existing nodes, which is what makes a rolling hardware upgrade expressible. `maxSubsystemCount` is unchanged: it stays the cluster's and every node's configuration is generated from it                                                                                                                                     |
...
type StorageClusterSpec struct {
	...
	// an explicit core count rather than a percentage. Unlike MaxSubsystemCount
	// it is stamped onto a node at creation, into StorageNode.spec.config.sizing,
