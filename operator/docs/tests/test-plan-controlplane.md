...
| U-148 | Ten consecutive failing probes: the failure is announced once in the log, not ten times   | Regression | `TestARepeatedFailingProbeIsNotAnnouncedAgain` |
...
| U-172     | `BuildingIndices` with no `Job` in the namespace: the `Job` is created, and the step holds naming it                                                                                                     | Positive | `TestBuildingIndicesHoldsUntilTheJobSucceeds`                         |
| U-173     | `BuildingIndices` with the `Job` carrying `Complete`: advances                                                                                                                                           | Positive | `TestBuildingIndicesHoldsUntilTheJobSucceeds`                         |
| ~~U-174~~ | ~~`BuildingIndices` with the `Job` carrying `Failed`: holds~~ Superseded by U-179                                                                                                                        | Negative | —                                                                     |
| U-175     | `BuildingIndices` with a `Job` already present under another image: left as it is, not re-applied                                                                                                        | Negative | `TestBuildingIndicesLeavesAJobThatAlreadyExistsAlone`                 |
| U-176     | The `Job` runs `sbctl cluster build-indices` on the spec's image, with `RestartPolicy: Never`, no service account of its own, and the cluster file                                                       | Positive | `TestTheIndexJobRunsTheBackfillAgainstTheDatabase`                    |
| U-177     | The `Job` against a database that demands a client certificate carries the FoundationDB certificate variables and mounts `simplyblock-foundationdb-tls`, not the serving certificate a later step issues | Positive | `TestTheIndexJobPresentsTheDatabasesPeerCertificate`                  |
| U-178     | A failed `Job` deleted by hand: the next pass creates it again and the backfill reruns                                                                                                                   | Boundary | —                                                                     |
| U-179     | `BuildingIndices` with the `Job` carrying `Failed` under the spec's image: `InstallationFailed`, `message` names the `Job`, the phase stays `Installing`, the step stays, and no requeue is scheduled    | Negative | `TestAFailedIndexJobFailsTheInstallation`                             |
| U-180     | `BuildingIndices` with a failed `Job` under an image the spec no longer names: the `Job` is removed, and the next pass creates one under the spec's image with no condition                              | Positive | `TestAFailedIndexJobIsReplacedWhenTheImageChanges`                    |
| U-181     | The `Job` carries `backoffLimit: 2`, so the backfill runs three times before it is reported failed                                                                                                       | Boundary | `TestTheIndexJobRunsTheBackfillAgainstTheDatabase`                    |
| U-182     | Every object the `Job`'s pod names, under mutual TLS, serving TLS alone, and plaintext: each is one an earlier step created or the FoundationDB operator wrote                                           | Positive | `TestTheIndexJobNamesNothingTheInstallHasNotCreatedYet`               |
...
| U-178                                                  | A failed index `Job` removed by hand                              | The recreate path is the not-found branch `U-172` covers, but no test deletes a failed `Job` and asserts the second creation, and that deletion is the one recovery an administrator has                |
| U-184                                                  | A backfill that stalls before it runs being reported              | The `Job` carries the deadline and the step fails on the condition, both under unit test, but only a live cluster runs the `Job` controller that turns a pod which is never created into that condition |
