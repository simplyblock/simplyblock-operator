...
| I-06 | No node in the cluster is `vdo-capable`: the pool still reconciles to a StorageClass, and a `VDOPoolUnsatisfiable` Warning event names the zero count | Negative   | —    |
| I-07 | A node becomes `vdo-capable`: the pool stops being reported as unsatisfiable on its next reconcile                                                    | Positive   | —    |
...
| E-09 | Every node in the cluster lacks capability: the PVC stays `Pending`, and the pool's `VDOPoolUnsatisfiable` event names the label nothing carries | Negative | —    |
| E-38 | A node gains capability without a `csi-node` restart: its label stays `false`, and the gate keeps volumes off it until the pod restarts          | Negative | —    |
...
| Trigger and actor             | The auto-detect probe, and an operator's hand-set label                                         | M-05, U-75 through U-79            | An administrator applying the out-of-service taint (design §14 Q4), a node gaining capability without a pod restart (E-38) |
...
| E-09                                                    | No capable node anywhere in the cluster                            | Not attempted, and design §5.1's pool event is not implemented (design §14 Q13), so the reason a PVC stays `Pending` is not yet assertable                                        |
