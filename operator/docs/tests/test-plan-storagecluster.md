...
| I-20 | `spec.deviceClass` omitted at creation: the stored object reads `NVMe`                                   | Boundary | —                 |
| I-21 | `spec.deviceClass` created as `LogicalBlock`, changed to `NVMe`: rejected as immutable                   | Negative | —                 |
...
| I-24 | `spec.deviceClass` omitted, then set to `LogicalBlock`: rejected, since the default is already the value | Boundary | —                 |
...
| I-02 … I-13, I-20 … I-24 | Every immutability and range rule                                  | Needs `envtest`, because CEL and `Required` are enforced by the API server and a fake client applies neither. `I-20` and `I-24` are the pair the `deviceClass` default creates: a defaulted field is never absent, so there is no first assignment to allow                                                                         |
...
|---------------------------|-------------------------|-------------------------------------------------|
| Namespace count           | Single namespace        | U-01 … U-67, I-01 … I-24, E-01 … E-12           |
