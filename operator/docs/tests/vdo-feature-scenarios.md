...
| 9 | `NodeStageVolume` failed when LVM saw a volume's two redundant NVMe-oF (HA) paths as separate local block devices with an identical on-disk PV signature | **Fixed and verified** (this session, commits `19b0de0`, `55e839d`, `a5609ef`) | Root cause chain, found across three rounds of live debugging: (1) `pvscan --cache <by-id-path>` did a broader-than-scoped scan and hit a genuine "duplicate PV" ambiguity between a volume's two HA device nodes — fixed by scoping every LVM command in `CreateOrAttachVDO`/`ResolveClonedVDO`/`GrowVDO` to the one intended device via `--devices`. (2) Even scoped, `vgExists`'s name-based `vgs --devices <path> <name>` lookup still reported a VG as existing when it had never been created on that device (this host restricts default LVM visibility via an `/etc/lvm/devices/system.devices` devices file) — fixed by switching to the same content-based `pvVGName` check `ResolveClonedVDO` already used. (3) With both fixes in place, both `vgExists`/`vgHasLV` correctly detected the two test volumes' VGs as genuinely **orphaned** — real leftover state from *before* fix #1, where `pvcreate`+`vgcreate` had completed but `lvcreate` never did — and correctly removed and recreated them from scratch. Live logs confirm all three fixes firing exactly as intended: `--devices` scoping, duplicate-PV warning gone, and `"VG ... exists but has no ... logical volume -- removing orphaned VG and recreating"` followed by a fresh, correct `lvcreate`. The very last step (`lvcreate`'s actual VDO index/cache memory allocation, ~446MiB) then hit a **separate, genuine hardware constraint** (Load-adjacent, not a code bug — see note below) on this small test cluster's VMs, so end-to-end mount success wasn't reachable in this session, but the code path itself is confirmed correct up to that point |
