...
// nothing else.
//
// Some of the tests here cannot run against a fake client: the CRD's CEL rules
// are evaluated by the apiserver, and structural defaulting only happens there
// too. That second half is the reason this suite exists at all — a field a fake
// client reports as false may be a field the apiserver never stored, and the
// difference is invisible until a CEL rule tries to read it.
//
// Everything else in this package is driven with a fake client, which is where
// the reconciler's stepwise behavior is provable and where the bulk of the
// coverage is.
//
// One apiserver serves all of them. It is started on the first test that asks
// for one and stopped when the package's tests finish, rather than started per
// test: envtest costs several seconds to come up, and four of them in one
// package is most of the package's runtime for no extra coverage.
...
// unreachable webhook rather than on anything the test is about — and v1alpha2
// is the storage version, which is what makes reading it need no conversion at
// all.
