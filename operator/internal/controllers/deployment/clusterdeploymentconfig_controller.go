...
// The document owns what the expansion raises from it, and an owner reference
// carrying blockOwnerDeletion needs update on the owner's finalizers wherever
// OwnerReferencesPermissionEnforcement runs — which is every OpenShift cluster,
// and no vanilla one. Without it the activate this controller creates is
// refused outright.
// +kubebuilder:rbac:groups=storage.simplyblock.io,resources=clusterdeploymentconfigs/finalizers,verbs=update
