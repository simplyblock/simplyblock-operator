...
// reconciler, so a control plane a ControlPlane.spec.source.managed points at
// elsewhere needs that entry to carry the endpoint this reconciler actually
// resolved, not a hardcoded address that only resolves inside its own
// cluster.
func TestCSICredentialsCarryTheControlPlanesResolvedEndpoint(t *testing.T) {
	...
	if got := creds.Clusters[0].ClusterEndpoint; got != hubEndpoint {
		t.Errorf("clusterEndpoint = %q, want the resolved endpoint %q", got, hubEndpoint)
	}
}
...
// An adopted cluster's control plane may be a ControlPlane.spec.source.managed
// one, on a different Kubernetes cluster, where a Kubernetes TokenReview of
// this operator's own service-account token can never succeed. The read that
// confirms the adoption must authenticate as the cluster itself instead,
// using the secret the upgrade Secret names.
func TestAnUpgradeSecretAuthenticatesTheAdoptionReadAsTheAdoptedCluster(t *testing.T) {
	// Every Cluster() call across the run is checked, not just the last: a
	// later, correctly-authenticated steady-state read would otherwise
	// overwrite a single captured value and hide a regression in the
	// adoption read specifically.
	var calls []struct {
		token string
		ok    bool
	}
	...
	for i, call := range calls {
		...
		if call.token != testClusterSecret {
			t.Errorf("call %d: bearer token = %q, want the cluster's own secret %q",
