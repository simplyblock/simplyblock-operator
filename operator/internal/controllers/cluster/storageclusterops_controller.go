...
func (r *StorageClusterOpsReconciler) advance(
	...
	// Every control-plane call this step and everything downstream of it makes
	// (perform, advanceWalk, and everything under them) authenticates as this
	// operation's own cluster when its secret is known, rather than as this
	// operator's Kubernetes identity -- the only way to reach a control plane a
	// different Kubernetes cluster runs (a shared hub), since a Kubernetes
...
// cluster runs (a shared hub), since a Kubernetes TokenReview can never cross a
// cluster boundary. See StorageClusterReconciler's identically-named method.
