...
// nodeAddress is the address a client mounts the export at, or "" when the
// node is gone or publishes none.
//
// §13.3 puts a Service in front of the metadata server so the address survives
// a move. That belongs with failover: until an export can move, a Service only
// absorbs a change that cannot happen.
