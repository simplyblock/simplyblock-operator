...
//
// Kept as a package-level helper rather than a method: all three gates that
// need it live on different reconcilers -- reconcileActivate below,
// maybeActivateCluster (simplyblockstoragenodeset_controller.go) and
// drainValidate (storagenodeops_controller.go).
...
func (r *StorageClusterReconciler) reconcileActivate(
	...
	// fires, and before any ActionStatus mutation, so a blocked activation
	// just requeues and re-checks next pass instead of latching
	// ActionStatus.State=Failed -- nothing in Reconcile/reconcileActivate
	// ever resets ActionStatus away from Failed back to Running, so failing
	// here would loop forever on the GET-polling tail without retrying the
	// POST. Mirrors the same gate in maybeActivateCluster
	// (simplyblockstoragenodeset_controller.go), which is the second,
	// independent path that fires this same POST. No-op for FD-disabled
	// clusters.
	if ptr.BoolFromOrFalse(clusterCR.Spec.EnableFailureDomains) {
		...
		if reason := fdActivationDomainCountViolation(npcs, hostDomains); reason != "" {
			...
			r.Recorder.Eventf(clusterCR, nil, corev1.EventTypeWarning, "FailureDomainNotReady", "FailureDomainNotReady",
