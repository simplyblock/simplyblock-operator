...
type PoolSpec struct {
	...
	// enforced when allowedHosts is non-empty
	// +kubebuilder:default=false
	DHCHAP bool `json:"dhchap,omitempty"`
	// +operator-sdk:csv:customresourcedefinitions:type=spec,displayName="Allowed Hosts"
	// AllowedHosts is the list of NVMe host NQNs that may access volumes in this pool.
	// When set, only listed hosts can connect and DHCHAP authentication
	// is enforced. When empty, any host can connect regardless of the dhchap flag.
	// Each entry is the NQN of a Kubernetes worker node. The NQN is generated by the
	// simplyblock-csi-node DaemonSet pod and can be read from that pod:
	//   kubectl -n simplyblock exec <simplyblock-csi-node-pod> -c csi-node -- cat /etc/nvme/hostnqn
	// (e.g. nqn.2014-08.org.nvmexpress:uuid:5e6a96d0-3955-41a9-98cf-1f6680581c7a)
