...
func recordFrom(object *unstructured.Unstructured) ExportRecord {
	...
	return ExportRecord{
		...
		// The Service's ClusterIP, not the bound host's own address
		// (status.mdsNodeIP): a client mounts this, and it does not change
		// when the export moves to a different host (design-pnfs-rwx.md
		// §13.3).
		ServiceAddress: get("status", "serviceAddress"),
