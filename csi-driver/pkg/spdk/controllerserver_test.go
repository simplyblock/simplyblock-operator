...
func TestHardPinTopologySegments(t *testing.T) {
	...
	t.Run("KNOWN LIMITATION: node in two DHCHAP pools gets both pools' labels ANDed together", func(t *testing.T) {
		...
		got := hardPinTopologySegments(req)
		want := map[string]string{poolKey: "allowed", otherPoolKey: "allowed"}
		if len(got) != len(want) || got[poolKey] != want[poolKey] || got[otherPoolKey] != want[otherPoolKey] {
			t.Errorf("got %v, want %v (documenting current, imprecise behavior)", got, want)
		}
	})
