...
func (ns *nodeServer) restageVolume(
	...
	// reformatting would destroy data. The filesystem is the one the volume was
	// staged with, which a claim annotation may have chosen over the requested one.
	fsType := stagedFsType(volumeContext, volCap)
	if err := ns.mounter.Mount(devicePath, stagingTargetPath, fsType, stagingMountFlags(fsType, volCap)); err != nil { //nolint:lll // unwrappable string/log/signature
