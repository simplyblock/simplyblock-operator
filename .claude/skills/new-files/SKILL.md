...
| `csi-driver/Makefile`, `cmd/main.go`, `internal/driver/driver.go` (3)                                               | the entry points, which name the upstream the driver was forked from                           | leave both copyright lines where they are: Arm's, and the `Copyright (c) Simplyblock GmbH` beneath it |
| every other `csi-driver` file (0)                                                                                   | the inherited headers have been cleared; what the driver carries now is simplyblock's own work | the pattern to copy                                                                                   |
