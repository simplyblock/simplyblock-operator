...
| 4   | Two `atlas-lib` helpers copied into `csi-driver/internal/util/nvmerepair.go:356,373` because the originals are unexported                                                           | exact body match against `nvmeof/inspect.go:498` and `nvmeof/repair.go:600` | 3    | Low: export and adopt. See `extract-to-atlas-lib`           |
...
| 8   | `BuildStorageNodeSetDaemonSet`, 311 lines at `operator/internal/utils/storage_nodeset_ds.go:48`                                                                                     | second longest                                                              | 6    | Low, and largely a flat literal, so split only reused parts |
...
| 12  | 52 `nolint` directives across the three modules                                                                                                                                     | each is a suppressed finding                                                | —    | Varies: read each one, since some are load-bearing          |
