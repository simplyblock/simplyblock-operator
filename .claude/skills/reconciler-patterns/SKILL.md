...
calls and reports both paths, and a grep reports neither.
...
| A transient failure worth retrying       | `ctrl.Result{}, err`, and controller-runtime backs off exponentially and logs it                 |
...
and six others exist, yet 86 call sites still write `RequeueAfter: 10 * time.Second`
inline. A named constant per controller is the whole ask, and the only way the
...
0. `scripts/check-reconcilers.py --changed` reports no errors, and its warnings
   ...
   below, and those stay a reading job.
...
4. Terminal state returns immediately, and the finalizer is removed on all paths.
5. The lock, if there is one, is released on all paths, only by its owner.
6. Requeue intervals are named constants, and requeue-against-error is deliberate.
