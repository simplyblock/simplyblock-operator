...
A positive scenario is one row. The negatives around it are mechanical — work the
...
| The gating flag is disabled                                    | The whole path is inert; nothing is written, no call made                |
...
| Field set to zero, negative, or the empty string    | Rejected or defaulted — say which                               |
...
| Immutable field mutated after creation              | Rejected; the running operation is unaffected                   |
...
| Referent is being deleted (`deletionTimestamp` set) | Classified as the design says; no half-action      |
...
| Mutation                                       | Row asserts                                          |
|------------------------------------------------|------------------------------------------------------|
| A second operation targets the same object     | Blocks or queues; no interleaving                    |
| Operations target two different objects        | Independent; no cross-locking                        |
| The lock holder dies without releasing         | Lock reclaimable; no permanent deadlock              |
...
| Two controllers act on the same backend object | One loses cleanly; the design names the coordination |

### Interruption

| Mutation                                         | Row asserts                                                   |
|--------------------------------------------------|---------------------------------------------------------------|
| Operator restarts, once per sub-phase            | Resumes from persisted state; the side effect is not repeated |
| The user cancels mid-operation                   | The target is restored to its pre-operation state             |
| The CR is deleted mid-operation                  | Finalizer completes the cleanup; nothing orphaned             |
...
- exactly at the threshold, and one ε either side (strict `>` vs `>=` — assert
  the documented one is what happens);
