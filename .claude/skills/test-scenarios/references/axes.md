...
value, and the rows it obliges. Walk all of them, and state which you excluded and
why.
...
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Single node**      | Every "pick another node" path has no answer. Degenerate: the only node is simultaneously hottest and coolest, `k > len(nodes)` must clamp, round-robin has no peer, drain has no migration target, and FTT is 0 so redundancy checks must not divide by zero. |
| **Two nodes**        | Tie-breaks and even splits, "picks the only available target," and one node offline leaving a single-node cluster mid-operation.                                                                                                                               |
...
- Single node: the feature no-ops cleanly, or fails with a clear reason and
  leaves the cluster untouched, never a nil deref and never a silent success.
...
- Five or more: the selection spreads instead of concentrating, and caps apply.
...
- A namespaced watcher sees an event in an unwatched namespace → ignored, no
  reconcile.
- RBAC: the controller's role covers every namespace it must reach, and a forbidden
...
**Applies always.** Values: **zero** (an empty list, the most-skipped row), one, a
...
exercises the no-tie-break path, and 100+ hits no CR-count, name-length, or API
