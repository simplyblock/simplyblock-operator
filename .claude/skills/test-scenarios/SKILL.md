...
before emitting a matrix; do not restate the format here. Scenario wording
...
output — an axis silently dropped reads as covered.
...
- **Cluster topology:** single-node, three-node, larger (5+). A single-node
  ...
  FTT=1 minimum where one failure puts the cluster at its limit; 5+ is where
...
- At least one `Boundary` row exists per behavior that compares, counts, or
  selects.
- Every selected axis value appears in ≥1 ID; every excluded axis has a stated
  reason.
...
write the design doc's Testing Strategy section — that stays a pointer.
