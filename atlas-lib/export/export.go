...
// Check reports whether the export is actually mounted and published, without
// changing anything on the host. A probe that repaired what it found broken
// would be a second, undeclared path to the mutations Create already owns.
//
// It does not check nfsd itself: nfsd is a kernel service shared by every
// export a host serves, not a property of one, and the wrapper that brings it
// up (csi-driver's nfsdAssembler) is what checks it.
func (a *Assembler) Check(ctx context.Context, spec Spec) error {
	if err := spec.Validate(); err != nil {
		return err
	}
	if err := a.checkMounted(ctx, spec); err != nil {
		return err
	}
	return a.checkPublished(ctx, spec)
}

// checkMounted reports whether spec.Path is a mount point right now. Not
// whether the drop-in file says it should be: a filesystem that was unmounted
// out from under nfsd, by a crash or by hand, leaves every file underneath
// reading as whatever the empty directory holds instead.
func (a *Assembler) checkMounted(ctx context.Context, spec Spec) error {
	out, code, err := a.cfg.Run(ctx, "mountpoint", "-q", spec.Path)
	if err != nil {
		return fmt.Errorf("export %s: running mountpoint: %w", spec.Path, err)
	}
	if code != 0 {
		return fmt.Errorf("export %s: not mounted: %s", spec.Path, strings.TrimSpace(string(out)))
	}
	return nil
}

// checkPublished reports whether nfsd's own table carries the export, which
// is what a client's mount actually depends on -- not the drop-in file this
// package wrote, which nfsd may have failed to read.
func (a *Assembler) checkPublished(ctx context.Context, spec Spec) error {
	out, code, err := a.cfg.Run(ctx, "exportfs", "-v")
	if err != nil {
		return fmt.Errorf("export %s: running exportfs -v: %w", spec.Path, err)
	}
	if code != 0 {
		return fmt.Errorf("export %s: exportfs -v exited %d: %s",
			spec.Path, code, strings.TrimSpace(string(out)))
	}
	if !strings.Contains(string(out), spec.Path) {
		return fmt.Errorf("export %s: not in nfsd's export table", spec.Path)
	}
	return nil
}
