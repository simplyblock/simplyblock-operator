...
// stubIdentifyMNAN replaces the Identify ioctl for the duration of a test,
// recording whether it was called and returning a fixed MNAN.
func stubIdentifyMNAN(t *testing.T, mnan uint32, err error) *bool {
	t.Helper()
	called := false
	prev := identifyMNAN
	identifyMNAN = func(devicePath string) (uint32, error) {
		called = true
		return mnan, err
	}
	t.Cleanup(func() { identifyMNAN = prev })
	return &called
}
...
func TestSubsystemIsMultiNamespace_ConclusiveFromSysfs(t *testing.T) {
	...
	for name, s := range cases {
		t.Run(name, func(t *testing.T) {
			called := stubIdentifyMNAN(t, 1, nil) // MNAN=1 would say "false" if consulted
...
func TestSubsystemIsMultiNamespace_IdentifyFallback(t *testing.T) {
	...
	t.Run("MNAN>1 is multi-namespace", func(t *testing.T) {
		called := stubIdentifyMNAN(t, 32, nil)
		got, err := s.IsMultiNamespace()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if !got {
			t.Errorf("IsMultiNamespace = false, want true (MNAN=32)")
		}
		if !*called {
	...
	t.Run("MNAN==1 is single-namespace", func(t *testing.T) {
		...
		if got {
			t.Errorf("IsMultiNamespace = true, want false (MNAN=1)")
		}
	})
}
...
func TestDeviceIsMultiNamespace_NSIDFastPath(t *testing.T) {
	...
	called := stubIdentifyMNAN(t, 1, nil)
