...
//
// A Device holds no handle back to the resolver it came out of, so it compares,
// copies, and crosses a wire as the value it looks like. A question it cannot
// answer from its own snapshot needs a fresh scan, which belongs to whatever
// holds the resolvers: see storage.Accessor.
//
// # Resolving devices
//
