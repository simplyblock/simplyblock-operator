...
//
// Everything here is a pure filter over a device snapshot the caller owns, and
// one List answers all four questions for every device in it. Asking them
// against current kernel state instead means re-scanning, because a namespace
// may have been attached since, and that is the job of whatever holds the
// resolvers, since that is also what knows what a rescan costs.
