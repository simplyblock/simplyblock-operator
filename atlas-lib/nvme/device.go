...
// own names, e.g., `SubsystemID("nvme-subsys0")` and `ControllerID("nvme0")`.
type (
	SubsystemID  string
	ControllerID string
...
type Subsystem struct {
	...
	Type        string // `subsystype`, e.g., `nvm` or `discovery`
	IOPolicy    string // `iopolicy`, e.g., `numa`, `round-robin`, or `queue-depth`
...
type Controller struct {
	...
	Address   Address // the parsed `address` attribute
	...
	// Reconnect and timeout tunables, in seconds. FastIOFailTMO may be `off`.
	KeepAliveTOSec    int    // `kato`
	CtrlLossTMOSec    int    // `ctrl_loss_tmo`
	ReconnectDelaySec int    // `reconnect_delay`
	FastIOFailTMO     string // `fast_io_fail_tmo`, `off` or a number of seconds
}
...
type Path struct {
	...
	ANAState   ANAState // `ana_state`, e.g., `optimized` or `non-optimized`
...
type Namespace struct {
	...
	// `nuse` attributes), independent of LogicalBlockSize. Multiply by 512
	// for bytes.
