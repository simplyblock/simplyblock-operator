...
func TestNarrowClampsRatherThanWrapping(t *testing.T) {
	// The values narrow is used on are small kernel integers, so the interesting
	// cases are the ones that cannot occur: without the clamp each would wrap to
	// a plausible-looking negative, which is worse than a saturated bound.
	for _, tc := range []struct {
		name string
		in   int
		want int32
	}{
		{"a NUMA node of -1 survives", -1, -1},
		{"zero", 0, 0},
		{"a typical timeout", 600, 600},
		{"the upper bound itself", math.MaxInt32, math.MaxInt32},
		{"the lower bound itself", math.MinInt32, math.MinInt32},
		// math.MaxInt exceeds int32 on a 64-bit platform and equals the bound on
		// a 32-bit one, so the expected answer is the bound either way.
		{"past the upper bound clamps, not wraps", math.MaxInt, math.MaxInt32},
		{"past the lower bound clamps, not wraps", math.MinInt, math.MinInt32},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := narrow(tc.in); got != tc.want {
				t.Errorf("narrow(%d) = %d, want %d", tc.in, got, tc.want)
			}
		})
	}
}
