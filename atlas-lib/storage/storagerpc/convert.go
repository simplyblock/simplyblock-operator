...
// narrow converts one of the small kernel integers sysfs reports as a Go int
// (a NUMA node, a queue count, a timeout in seconds, a command-set id) to the
// int32 the wire carries it in. None of them can reach the bound in practice,
// so the clamp never fires; it is here because the conversion cannot be proven
// safe from the types alone, and a silent wrap would turn a large value into a
// negative one, which reads as a legitimate answer.
...
func controllerToProto(c nvme.Controller) *storagev1.Controller {
	return &storagev1.Controller{
		...
		HostNqn:  c.HostNQN,
		HostId:   c.HostID,
		NumaNode: narrow(c.NUMANode),

		QueueCount: narrow(c.QueueCount),
		SqSize:     narrow(c.SQSize),

		KeepAliveToSec:    narrow(c.KeepAliveTOSec),
		CtrlLossTmoSec:    narrow(c.CtrlLossTMOSec),
		ReconnectDelaySec: narrow(c.ReconnectDelaySec),
