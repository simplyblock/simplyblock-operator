...
func (s *stubAPI) QueryRange(
	_ context.Context, query string, r promv1.Range, _ ...promv1.Option,
) (model.Value, promv1.Warnings, error) {
	s.queries = append(s.queries, query)
...
// series builds one range-query series from its values, timestamped at the step
// the tests use. Only the values and their order are asserted on, so the
// timestamps only have to be distinct and increasing.
func series(labels map[string]string, values ...float64) *model.SampleStream {
	m := model.Metric{}
	for k, v := range labels {
		m[model.LabelName(k)] = model.LabelValue(v)
	}
	...
	for i, v := range values {
		pairs = append(pairs, model.SamplePair{
			Timestamp: model.Time(int64(i) * int64(5*time.Minute/time.Millisecond)),
			Value:     model.SampleValue(v),
		})
	}
	return &model.SampleStream{Metric: m, Values: pairs}
}
...
// A windowed read keeps every sample of a node's series in order, because the
// baseline it feeds rejects outliers and so needs the readings rather than a
// reduction of them.
func TestClusterLatencySamplesKeepsEverySampleInTheWindow(t *testing.T) {
	api := &stubAPI{matrix: model.Matrix{
		series(map[string]string{"cluster": "c1", "node": "n1"}, 1100, 1150, 1120),
		series(map[string]string{"cluster": "c1", "node": "n2"}, 2200),
	}}

	got, err := NewWithAPI(api).ClusterLatencySamples(
		context.Background(), []string{"c1"}, PercentileP50, 6*time.Hour, 5*time.Minute,
	)
	if err != nil {
		t.Fatalf("ClusterLatencySamples: %v", err)
	}
	if want := []float64{1100, 1150, 1120}; !slices.Equal(got["c1"]["n1"], want) {
		t.Errorf("n1 = %v, want %v", got["c1"]["n1"], want)
	}
	if want := []float64{2200}; !slices.Equal(got["c1"]["n2"], want) {
		t.Errorf("n2 = %v, want %v", got["c1"]["n2"], want)
	}
	// The window and the step are what distinguish this from an instant read,
	// so they have to reach the range query rather than being defaulted away.
	if r := api.ranges[0]; r.End.Sub(r.Start) != 6*time.Hour || r.Step != 5*time.Minute {
		t.Errorf("range = %v..%v step %v, want a 6h window at a 5m step", r.Start, r.End, r.Step)
	}
}

// No clusters is not a range query either, for the same reason an instant read
// of none is not.
func TestClusterLatencySamplesWithNoClustersIssuesNoQuery(t *testing.T) {
	api := &stubAPI{}
	got, err := NewWithAPI(api).ClusterLatencySamples(
		context.Background(), nil, PercentileP50, time.Hour, time.Minute,
	)
	if err != nil {
		t.Fatalf("ClusterLatencySamples: %v", err)
	}
