...
// queryMatrix runs a range query over [now-window, now] and requires the result
// to be a matrix: one series per entity, each carrying every sample the step
// resolution puts in the window. It is the shape a caller that reduces a window
// to one robust number needs, and an instant query cannot give it.
func (p *Provider) queryMatrix(
	...
	now := time.Now()
	val, _, err := p.api.QueryRange(ctx, query, promv1.Range{
