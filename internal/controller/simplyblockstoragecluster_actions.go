...
func (r *StorageClusterReconciler) reconcileRestart(
	...
	if !clusterCR.Status.ActionStatus.Triggered {
		...
		body, status, err := apiClient.Do(ctx, http.MethodPost, apiEndpoint, nil)
		if err != nil || status >= 300 {
			if err == nil {
				err = fmt.Errorf("unexpected status %d body=%s", status, string(body))
			}
...
func (r *StorageClusterReconciler) reconcileNodeRecycle(
	...
	case utils.NodeRecyclePhaseSnodeRefresh:
		return r.nodeRecycleSnodeRefresh(ctx, clusterCR, apiClient, clusterUUID, currentNodeUUID)
	case utils.NodeRecyclePhaseSnodeRefreshWait:
		return r.nodeRecycleSnodeRefreshWait(ctx, clusterCR, apiClient, clusterUUID, currentNodeUUID)
	case utils.NodeRecyclePhaseShuttingDown:
		return r.nodeRecycleShuttingDown(ctx, clusterCR, apiClient, clusterUUID, currentNodeUUID)
	case utils.NodeRecyclePhaseRestarting:
		return r.nodeRecycleRestarting(ctx, clusterCR, apiClient, clusterUUID, currentNodeUUID)
	case utils.NodeRecyclePhaseRebalancing:
		return r.nodeRecycleRebalancing(ctx, clusterCR, apiClient, clusterUUID, currentNodeUUID)
	default:
...
func (r *StorageClusterReconciler) nodeRecycleShuttingDown(
	...
	if !nrs.PhaseTriggered {
		...
		if !alreadyShuttingDown {
			// Persist PhaseTriggered=true before the API call so a crash between
			// the call and the persist cannot trigger a duplicate shutdown on restart.
			...
			if err != nil || status >= 300 {
				...
				// Revert PhaseTriggered so the call is retried on the next reconcile.
				nrs.PhaseTriggered = false
				if updateErr := r.Status().Update(ctx, clusterCR); updateErr != nil {
					log.Error(updateErr, "Failed to revert PhaseTriggered after shutdown API failure")
				}
				return ctrl.Result{RequeueAfter: 10 * time.Second}, nil
			}
	...
	case utils.NodeStatusOnline:
		// Node is already online — the shutdown+restart cycle completed while the
		// operator was down. Skip straight to rebalancing so the node is marked
		// processed and the recycle continues for remaining pending nodes.
...
func (r *StorageClusterReconciler) nodeRecycleRestarting(
	...
	if !nrs.PhaseTriggered {
		...
		if !alreadyRestarting {
			// Persist PhaseTriggered=true before the API call so a crash between
			// the call and the persist cannot trigger a duplicate restart on restart.
			nrs.PhaseTriggered = true
			if err := r.Status().Update(ctx, clusterCR); err != nil {
				return ctrl.Result{Requeue: true}, nil
			}
			...
			if err != nil || status >= 300 {
				...
				nrs.PhaseTriggered = false
				if updateErr := r.Status().Update(ctx, clusterCR); updateErr != nil {
					log.Error(updateErr, "Failed to revert PhaseTriggered after restart API failure")
				}
				return ctrl.Result{RequeueAfter: 10 * time.Second}, nil
			}
