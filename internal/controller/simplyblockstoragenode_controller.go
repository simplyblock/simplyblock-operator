...
var (
	...
	waitForNodeOnlineActivationDelay = 30 * time.Second
...
func (r *StorageNodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, nodeName := range snCR.Spec.WorkerNodes {
		...
		// that have been confirmed online at least once. For socket configs the
		// control-plane creates expectedPerHost nodes per physical host, so we
		// skip only when all of them are tracked.
		trackedCount := 0
		...
		if !hasPlaceholder {
			...
			// Add a single placeholder entry so we know the POST was sent.
...
func waitForNodeOnline(
	...
	for attempt := 1; attempt <= waitForNodeOnlineRetries; attempt++ {
		...
		// All socket nodes are online — sync status entries keyed by UUID.
		...
		for _, res := range onlineForHost {
			...
			if !matched {
				// New socket node not yet in status — append it.
