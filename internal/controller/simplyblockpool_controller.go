...
func (r *SimplyBlockPoolReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if pool.Status.UUID == "" {
		params := utils.PoolAddParams{
			...
			VolumeMaxSize: 0,
			MaxRwMB:       poolSpecQoSThroughputReadWrite(poolCR.Spec.QoS),
			MaxRwIOPS:     poolSpecQoSIOPS(poolCR.Spec.QoS),
			MaxRMB:        poolSpecQoSThroughputRead(poolCR.Spec.QoS),
			MaxWMB:        poolSpecQoSThroughputWrite(poolCR.Spec.QoS),
			CRName:        poolCR.Name,
